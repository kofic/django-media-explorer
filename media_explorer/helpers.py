...
class FieldHelper(object):
    ...
    def on_mediaimagefield_post_save_callback(self, **kwargs):
        """
        ...
        if type(instance.__dict__[field_name]) in [str, unicode]:
            image_url = instance.__dict__[field_name]
        elif hasattr(instance.__dict__[field_name], "url"):
            image_url = instance.__dict__[field_name].url
        ...
        data = {}
        data["image"] = instance.__dict__[field_name]
        element = Element()
        element.__dict__.update(data)
        element.s3_is_public = s3_is_public
        element.thumbnail_s3_is_public = s3_is_public
        element.save()
